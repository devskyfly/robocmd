...
trait DevTestTrait 
{
    ...
    public function testsRun($opts = ["suite|s" => "all", "debug|d" => false, "localserver|l" => false])
    {
        ...
        if ($opts['suite'] == "all" || $opts['suite'] == "unit") {
            $collection->addTask($this->taskExec(getcwd().'/vendor/bin/codecept run unit'.($opts['debug']?" --debug":"").($opts['verbose']?" -v":"")));
        }

        if ($opts['suite'] == "all" || $opts['suite'] == "functional") {
            $collection->addTask($this->taskExec(getcwd().'/vendor/bin/codecept run functional'.($opts['debug']?" --debug":"").($opts['verbose']?" -v":"")));
        }
